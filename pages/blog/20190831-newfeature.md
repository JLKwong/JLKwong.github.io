...
1. Your first line will define a function which we will call faketemp
...
For example `treehouses faketemp` will return the temperature in a default setting, but `treehouses faketemp fahrenheit` will return it in Fahrenheit. The command `fahrenheit` is your subcommand.
...
Now instead of your command being just `treehouses faketemp` it can now also be `treehouses faketemp fahrenheit` or `treehouses faketemp kelvin`
...
1. **README.md**: First you will include your command in the `README.md` file in the cli Repo
    ...
    * Your entry should look like this: `echo "  faketemp [fahrenheit|kelvin]     displays raspberry pi's CPU temperature"`

2. **help.sh**: Now go into cli/modules and modify the `help.sh` file in the exact same way you modified the `README.md` file

3. **Modules**: Now you will add your `faketemp.sh` file into the modules folder. This is the file that has the source code for your new feature.
    * Inside the file you will make sure you have a second function called function `faketemp_help`
    ...
    * Your final `faketemp.sh` file should look like this:
...
## Step 4: Testing Your Command
 ...
 _**IMPORTANT NOTE:**_ When creating a branch, make sure it uses the feature you are adding or updating to be the prefix. For example, here we are creating the  `faketemp` module with subcommands `fahrenheit` and `kelvin`. So a good name for this branch could be: `faketemp-conversions`
 
  1. Enter `git branch` for safe measure to see that you really are on your branch
  1. Now you can test out your new command and subcommands: `treehouses faketemp`, `treehouses faketemp fahrenheit`, and `treehouses faketemp kelvin`
  1. You may have to use `./cli.sh` instead of `treehouses` until your branch is approved: `./cli.sh faketemp`
  1. If your command didn't work the way you intended it to, you can always fix your files
  1. Cd into cli/modules using `cd modules` and open your desired file using vim: for example `vim faketemp.sh`
  1. You can now modify your file by pressing `i` and exiting by pressing esc then typing `:wq`
  1. Test your file again using `./cli.sh faketemp` until it works how you intended it to
